...
> **Naming map:** Argus (brand) ≡ `qa-agent` repo ≡ factory `agent=qa`. Pantheon siblings live elsewhere: Hephaestus (dev) and Themis (review) in the app repo — not renamed here. Internal paths, skills, and scripts keep the `qa-*` prefix. Teams Adaptive Cards use title colour **`Warning`** (Hephaestus=`Accent`, Athena=`Good`) — keep the three colours distinct (RQ-1957).
