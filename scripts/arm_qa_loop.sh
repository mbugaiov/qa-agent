...
PROMPT="EXECUTE qa-loop tick NOW for ${SLUG} - full checklist every time, no notify-only mode exists regardless of the wake title or wording: tick_start -> qa_scope --log --shell -> per-ticket handoff+OpenSpec+TC+DoD -> close (transition + jira_close_issue.py/jira_return_in_progress.py comment) -> evidence (recording/screenshot) -> new/updated regression tests -> drain the backlog (re-run qa_scope.sh again and keep working ticket by ticket, including impl-qa To Do, until a scan returns count=0 or only dev-owned SKIP_DEV remains, then log factory_log.sh backlog_drained) -> factory_tick_gate.sh -> exploratory -> tick_end -> run.md. Do the drain-and-log-backlog_drained step BEFORE factory_tick_gate.sh, not after - do not stop at one ticket per tick. Forbidden: notify-only or status-only replies, no matter how the wake is titled."
