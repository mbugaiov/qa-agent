...
# Tracker: Jira (multipart attach) or GitHub Issues (qa-evidence branch +
# inline markdown previews via project GITHUB_TOKEN).
...
if [[ "$PROVIDER" == "github_issues" ]]; then
  ...
  echo "Attached retest recording to $KEY (${MB}MB) — viewable inline on GitHub. Local copy discarded."
  exit 0
fi
