...
#
# Usage:
#   scripts/ticket_tc.sh <slug> <TICKET> --title "…" [--steps-file path] [--scenario SC-*] [--req REQ-*] [--log]
...
TICKET="${2:-}"
[[ -z "$SLUG" || -z "$TICKET" ]] && {
  echo "Usage: ticket_tc.sh <slug> <TICKET> [--title …] [--link TC-id] [--steps-file path] [--log]" >&2
  exit 1
}
shift 2
