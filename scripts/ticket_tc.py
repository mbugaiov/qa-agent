...
def upsert_regression_index(project: Path, ticket: str, tc_id: str, rel_path: str) -> None:
    ...
    if REGRESSION_SECTION not in text:
        ...
        anchor = "## Coverage ledger"
        if anchor in text:
            text = text.replace(anchor, block + anchor, 1)
        else:
            text = text.rstrip() + block
    else:
        if ticket not in text.split(REGRESSION_SECTION, 1)[1].split("##", 1)[0]:
            insert_at = text.find("|------|-----|------|------------|")
            if insert_at != -1:
                line_end = text.find("\n", insert_at)
...
def main() -> int:
    ...
    if existing:
        ...
        if a.log:
            log_tc_linked(slug, ticket, tc_id, existing, created=False)
        return 0
    ...
    if a.link:
        ...
        if not path.is_file():
            path = next(test_cases_dir(project).rglob(f"{tc_id}.md"), None) if test_cases_dir(project).is_dir() else None
        if not path or not path.is_file():
            print(f"ERROR: TC file not found for --link {a.link}", file=sys.stderr)
            return 1
        text = path.read_text(encoding="utf-8")
        if not JIRA_LINE.search(text):
            if "## Expected" in text:
                text = text.replace("## Expected", f"- **Jira:** {ticket}\n\n## Expected", 1)
            else:
                text = text.rstrip() + f"\n- **Jira:** {ticket}\n"
            path.write_text(text, encoding="utf-8")
        rel = path.relative_to(project)
        upsert_regression_index(project, ticket, path.stem, str(rel))
    ...
    rel = path.relative_to(project)
    upsert_regression_index(project, ticket, tc_id, str(rel))
