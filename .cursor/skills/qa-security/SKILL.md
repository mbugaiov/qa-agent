...
| Outcome | Action |
|---------|--------|
| **Confirmed defect** | File under epic/repo via `create_bug_issue.py` with `--labels <slug>,confirmed-defect` (auto-adds `impl-dev`; dedupe first); severity per `qa-team` scale; attach evidence |
